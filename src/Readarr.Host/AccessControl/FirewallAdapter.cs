...
namespace Readarr.Host.AccessControl
{
    ...
    public class FirewallAdapter : IFirewallAdapter
    {
        ...
        public void MakeAccessible()
        {
            ...
            {
                if (!IsReadarrPortOpen(_configFileProvider.Port))
                {
                    _logger.Debug("Opening Port for Readarr: {0}", _configFileProvider.Port);
                    OpenFirewallPort(_configFileProvider.Port);
                }

                if (_configFileProvider.EnableSsl && !IsReadarrPortOpen(_configFileProvider.SslPort))
                {
        ...
        private bool IsReadarrPortOpen(int port)
        {
            try
            {
