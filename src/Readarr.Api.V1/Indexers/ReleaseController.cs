    ...
    public class ReleaseController : ReleaseControllerBase
    {
        ...
        public async Task<object> DownloadRelease([FromBody] ReleaseResource release)
        {
            ...
            if (remoteEpisode == null)
            {
                ...
                throw new ReadarrClientException(HttpStatusCode.NotFound, "Couldn't find requested release in cache, try searching again");
            }
            ...
            try
            {
                ...
                if (remoteEpisode.Series == null)
                {
                    ...
                    else if (release.SeriesId.HasValue)
                    {
                        ...
                        if (episodes.Empty())
                        {
                            throw new ReadarrClientException(HttpStatusCode.NotFound, "Unable to parse episodes in the release, will need to be manually provided");
                        }
                    ...
                    {
                        throw new ReadarrClientException(HttpStatusCode.NotFound, "Unable to find matching series and episodes, will need to be manually provided");
                    }
                }
            ...
            catch (ReleaseDownloadException ex)
            {
                ...
                throw new ReadarrClientException(HttpStatusCode.Conflict, "Getting release from indexer failed");
            }
        ...
        private async Task<List<ReleaseResource>> GetEpisodeReleases(int episodeId)
        {
            ...
            {
                throw new ReadarrClientException(HttpStatusCode.BadRequest, ex.Message);
            }
            catch (Exception ex)
            {
                _logger.Error(ex, "Episode search failed: " + ex.Message);
                throw new ReadarrClientException(HttpStatusCode.InternalServerError, ex.Message);
            }
        }
