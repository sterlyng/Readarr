    ...
    public class UpdateServiceFixture : CoreTest<InstallUpdateService>
    {
        ...
        public void Setup()
        {
            if (OsInfo.IsLinux)
            {
                ...
                {
                    FileName = "Readarr.develop.2.0.0.0.tar.gz",
                    Url = "http://download.Readarr.tv/v2/develop/mono/Readarr.develop.tar.gz",
            ...
            else
            {
                _updatePackage = new UpdatePackage
                {
                    FileName = "Readarr.develop.2.0.0.0.zip",
                    Url = "http://download.Readarr.tv/v2/develop/windows/Readarr.develop.zip",
                    Version = new Version("2.0.0.0")
                };
            }
        ...
        {
            Mocker.GetMock<IAppFolderInfo>().SetupGet(c => c.StartUpFolder).Returns(@"C:\Readarr".AsOsAgnostic);
            Mocker.GetMock<IAppFolderInfo>().SetupGet(c => c.AppDataFolder).Returns(@"C:\Readarr".AsOsAgnostic);
