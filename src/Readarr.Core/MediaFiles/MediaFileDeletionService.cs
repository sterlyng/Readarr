                                            ...
                                            IHandle<EpisodeFileDeletedEvent>
    {
        ...
        public void DeleteEpisodeFile(Series series, EpisodeFile episodeFile)
        {
            ...
            if (!_diskProvider.FolderExists(rootFolder))
            {
                ...
                throw new ReadarrClientException(HttpStatusCode.Conflict, "Series' root folder ({0}) doesn't exist.", rootFolder);
            }

            if (_diskProvider.GetDirectories(rootFolder).Empty())
            {
                _logger.Warn("Series' root folder ({0}) is empty.", rootFolder);
                throw new ReadarrClientException(HttpStatusCode.Conflict, "Series' root folder ({0}) is empty.", rootFolder);
            }
            ...
            if (_diskProvider.FolderExists(series.Path) && _diskProvider.FileExists(fullPath))
            {
                ...
                catch (Exception e)
                {
                    ...
                    throw new ReadarrClientException(HttpStatusCode.InternalServerError, "Unable to delete episode file");
                }
            }
