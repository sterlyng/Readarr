...
namespace Readarr.Core.Validation
{
    public static class RuleBuilderExtensions
    {
        ...
        {
            return ruleBuilder.WithState(v => ReadarrValidationState.Warning);
        }

        public static IRuleBuilderOptions<T, string> StartsOrEndsWithReadarr<T>(this IRuleBuilder<T, string> ruleBuilder)
        {
            ruleBuilder.SetValidator(new NotEmptyValidator(null));
            return ruleBuilder.SetValidator(new RegularExpressionValidator("^Readarr|Readarr$")).WithMessage("Must start or end with Readarr");
        }
    }
}
