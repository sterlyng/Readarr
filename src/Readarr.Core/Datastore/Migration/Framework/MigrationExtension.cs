...
namespace Readarr.Core.Datastore.Migration.Framework
{
    public static class MigrationExtension
    {
        ...
        public static IMigrationRunnerBuilder AddReadarrSQLite(this IMigrationRunnerBuilder builder, bool binaryGuid = false, bool useStrictTables = false)
        {
            builder.Services
                .AddTransient<SQLiteBatchParser>()
                .AddScoped<SQLiteDbFactory>()
                .AddScoped<ReadarrSQLiteProcessor>(sp =>
                {
                    var factory = sp.GetService<SQLiteDbFactory>();
                    var logger = sp.GetService<ILogger<ReadarrSQLiteProcessor>>();
                    ...
                    return new ReadarrSQLiteProcessor(factory, sp.GetService<SQLiteGenerator>(), logger, options, connectionStringAccessor, sp, sqliteQuoter);
                })
                .AddScoped<ISQLiteTypeMap>(_ => new ReadarrSQLiteTypeMap(useStrictTables))
                .AddScoped<IMigrationProcessor>(sp => sp.GetRequiredService<ReadarrSQLiteProcessor>())
                .AddScoped(
                    sp =>
                    {
