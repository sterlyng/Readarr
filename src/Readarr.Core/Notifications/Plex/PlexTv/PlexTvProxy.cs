...
namespace Readarr.Core.Notifications.Plex.PlexTv
{
    ...
    public class PlexTvProxy : IPlexTvProxy
    {
        ...
        private string ProcessRequest(HttpRequestBuilder requestBuilder)
        {
            ...
            {
                throw new ReadarrClientException(ex.Response.StatusCode, "Unable to connect to plex.tv");
            }
            catch (WebException)
            {
                throw new ReadarrClientException(HttpStatusCode.BadRequest, "Unable to connect to plex.tv");
            }
