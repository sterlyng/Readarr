    ...
    public class AppFolderFactory : IAppFolderFactory
    {
        ...
        private void MigrateAppDataFolder()
        {
            try
            {
                if (OsInfo.IsOsx)
                {
                    var userAppDataFolder = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.UserProfile, Environment.SpecialFolderOption.DoNotVerify), ".config", "Readarr");
                ...
                var oldDbFile = Path.Combine(_appFolderInfo.AppDataFolder, "Readarr.db");
            ...
            catch (Exception ex)
            {
                ...
                throw new ReadarrStartupException(ex, "Unable to migrate AppData folder from {0} to {1}. Migrate manually", _appFolderInfo.LegacyAppDataFolder, _appFolderInfo.AppDataFolder);
            }
        }
        ...
        private void RemovePidFile()
        {
            ...
            {
                _diskProvider.DeleteFile(Path.Combine(_appFolderInfo.AppDataFolder, "Readarr.pid"));
            }
        }
    }
}
